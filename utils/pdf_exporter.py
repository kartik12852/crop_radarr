...
Produces a professionally formatted PDF with zone risk summary,
validation metrics, and actionable recommendations.
"""
...
class ClimateReportPDF:
    ...
    def generate_report(
        ...
        self._header(
            "Crop Climate Risk Assessment Report",
        ...
        for rid, cnt in risk_counts.items():
            self._bullet(
                f"{LABEL_NAMES.get(int(rid), str(rid))}: "
        ...
        if metrics:
            ...
            for k, v in metrics.items():
                if k not in ("runtime", "model_family"):
        ...
        p       = self.pdf
        col_w   = [55, 28, 35, 22, 22]
        headers = ["Zone Name", "Crop", "Risk Name", "Conf %", "Severity"]
        p.set_font("Helvetica", "B", 9)
        p.set_fill_color(200, 230, 200)
        ...
        for idx, (_, row) in enumerate(zones_df.iterrows()):
            ...
            p.set_fill_color(*c)
            p.cell(col_w[2], 6, rname,  border=1, fill=True)
            p.set_fill_color(255, 255, 255)
            p.cell(col_w[3], 6, conf,   border=1)
            p.cell(col_w[4], 6, sev,    border=1)
            p.ln()

        # Recommendations
        if recommendations:
            self.pdf.add_page()
            self._header("Zone-wise Mitigation Recommendations", f"Generated: {ts}")
            for rec in recommendations[:12]:
                self._section(
                    f"{rec['zone_name']} \u2014 {rec['crop']} \u2014 {rec['risk_name']}"
                )
                for act in rec.get("immediate", [])[:3]:
                    self._bullet(act, indent=14)
                if rec.get("crop_tip"):
                    self._kv("Crop Tip", rec["crop_tip"])
                self.pdf.ln(2)
        ...
        p.cell(0, 6, "Climate Radar System | Predictions are advisory only — verify with local agronomist.", align="C")
...
def generate_pdf_report(
    ...
    return ClimateReportPDF().generate_report(
        zones_df, preds, probs, metrics, recommendations
    )